...
PYBIND11_MODULE(schrodinger, m) {
    ...
    struct KeepAliveEigenfunction {
        std::unique_ptr<Schrodinger2D<double>::Eigenfunction> eigenfunction;
    ...
    py::class_<KeepAliveEigenfunction>(m, "Eigenfunction2D")
            ...
            .def("__call__", [](const KeepAliveEigenfunction &f, Schrodinger2D<double>::ArrayXs xs,
    ...
    py::class_<Schrodinger2D<double>, std::shared_ptr<Schrodinger2D<double>>>(m, "Schrodinger2D")
            ...
            .def("eigenfunctions", [](std::shared_ptr<const Schrodinger2D<double>> s) {
                std::vector<std::pair<double, std::unique_ptr<Schrodinger2D<double>::Eigenfunction>>> eigs = s->eigenfunctions();
                py::list r;
                for (auto &ef: eigs) {
                    py::tuple t(2);
                    t[0] = ef.first;
